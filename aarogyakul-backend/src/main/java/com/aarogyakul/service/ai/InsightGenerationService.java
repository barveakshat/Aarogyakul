...
public class InsightGenerationService {
    ...
    private static final String MEDICAL_DISCLAIMER =
            "\n\n⚕\uFE0F This summary is generated by AI and is not medical advice. Consult a qualified doctor before making any health decisions.";
    ...
    public String generate(List<ComparisonData> comparisons) {
        try {
            ...
            return llamaClient.chat(SUMMARY_PROMPT, json, 600).trim() + MEDICAL_DISCLAIMER;
        } catch (Exception e) {
    ...
    private String fallbackSummary(List<ComparisonData> comparisons) {
        if (comparisons.isEmpty()) {
            return "No lab parameters could be extracted confidently from this report." + MEDICAL_DISCLAIMER;
        }
        var first = comparisons.getFirst();
        if (first.previousValue() == null) {
            return ("I extracted %d lab parameter(s) from this report. This is the first stored result for %s, so future reports can be compared against it."
                    .formatted(comparisons.size(), first.parameterName())) + MEDICAL_DISCLAIMER;
        }
        return ("%s changed from %s to %s. Consider discussing notable changes with your doctor, especially if this differs from your usual range."
                .formatted(first.parameterName(), first.previousValue(), first.currentValue())) + MEDICAL_DISCLAIMER;
    }
}
